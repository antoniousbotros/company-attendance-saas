...
export default function BillingPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <SectionCard className="bg-[#f9fafb]">
          <div className="text-start">
            ...
            <StatusPill
              label={company.subscription_status === "active" ? t.active : t.activeTrial}
              tone="success"
            />
          </div>
        </SectionCard>
        ...
        <SectionCard>
          <div className="text-start">
            ...
            <p className="text-[10px] text-[#9ca3af] mt-1 font-medium">{isRTL ? `${EXTRA_EMPLOYEE_COST} جنيه/موظف إضافي` : `${EXTRA_EMPLOYEE_COST} EGP/extra employee`}</p>
          </div>
        </SectionCard>
      ...
      {/* All Features Banner */}
      <SectionCard className="bg-[#f9fafb]">
        <p className="text-sm font-bold text-[#111] mb-3 text-start">{isRTL ? "جميع الميزات متاحة في كل الباقات" : "All features included in every plan"}</p>
      ...
      {/* Plan Cards */}
      <div>
        <h2 className="text-lg font-bold text-[#111] mb-4 text-start">{isRTL ? "اختر باقتك" : "Choose your plan"}</h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              >
                ...
                <div className="text-start flex-1">
                  ...
                  <div className="mt-1 flex items-baseline gap-1">
                    {plan.price === 0 ? (
                      <span className="text-2xl font-black text-[#1e8e3e]">{isRTL ? "مجاني" : "Free"}</span>
                    ) : (
                      <>
                        <span className="text-2xl font-black text-[#111]">{plan.price}</span>
        ...
        <p className="text-xs text-[#6b7280] font-medium mt-3 text-start">
          {isRTL
            ...
            : `Need more employees? Add extra employees for ${EXTRA_EMPLOYEE_COST} EGP/month each.`}
        </p>
      </div>
